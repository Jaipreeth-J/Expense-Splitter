...
export default function GroupDetailPage() {
  ...
  async function loadGroupDetails() {
    ...
    setError('');
    try {
      // Members come from the balances endpoint (Day 6) — it already returns
      // userId/name/email per member. A dedicated "list members" endpoint
      // wasn't built in the original API design, so this doubles up for now.
      const balancesData = await apiFetch(`/groups/${id}/balances`);
      setMembers(balancesData.balances);

      // Fill in the group name if we didn't get it from navigation state
      // (e.g. the page was loaded directly via URL / refresh)
      if (!groupName) {
